...
if [[ "$major" -eq 3 && "$minor" -ge 9 ]]; then
    ...
    echo -e "${ORANGE}Current version of Python ($version_number) is too old for ${service_name}.${NC}"
...
if [[ -f /etc/os-release ]]; then
    ...
    if [[ "$ID_LIKE" == *"debian"* || "$ID" == *"debian"* ]]; then
        ...
        echo -e "${ORANGE}Your operating system is not supported. Either try to install manually or reach out to ${service_name} github for support.${NC}"
        exit 1
    fi
fi
...
#
echo -e "${GREEN}Installing system-wide pip packages needed for ${service_name}...${NC}"
if [[ $is_externally_managed_env ]]; then
...
echo -e "${GREEN}Installing ${service_name} backend and its dependencies...${NC}"
...
#
echo -e "${CYAN}Do you want to install ${service_name} as a systemd service? This will automatically start ${service_name} when your server starts. (y/n)${NC}"
read choice
...
if [ "$choice" == "y" ] || [ "$choice" == "Y" ]; then
    ...
    if [ -f "$systemd_user_dir/$service_name.service" ]; then
        ...
        systemctl --user stop $service_name  # Stop the service if it's running
        systemctl --user disable $service_name  # Disable the service
    ...
    # Verify that we found the right service dir by checking for the existence of pyproject.toml
    if [ ! -f "$service_dir/pyproject.toml" ]; then
        echo -e "${ORANGE}Could not automatically find the ${service_name} directory. Please specify the path to the ${service_name} directory (the directory containing pyproject.toml):${NC}"
        read service_dir
        # Expand the path
        service_dir=$(eval echo "$service_dir")
        # Verify again
        if [ ! -f "$service_dir/pyproject.toml" ]; then
            echo -e "${ORANGE}Could not find pyproject.toml in $service_dir. Aborting installation.${NC}"
            exit 1
        fi
    fi
...
[Service]
Type=simple
ExecStart=bash $service_dir/scripts/start.sh
...
"
    ...
    echo -e "${GREEN}${service_name} systemd service has been installed and ${service_name} is now starting.${NC}"
    echo -e "${GREEN}${service_name} will soon be reachable at ${ORANGE}http://$local_ip:$SPOOLMAN_PORT${NC}"
    ...
    echo -e "${GREEN}You can start/restart/stop the service by running e.g. '${CYAN}sudo systemctl stop ${service_name}${GREEN}'${NC}"
    echo -e "${GREEN}You can disable the service from starting automatically by running '${CYAN}sudo systemctl disable ${service_name}${GREEN}'${NC}"
    echo -e "${GREEN}You can view the ${service_name} logs by running '${CYAN}sudo journalctl -u ${service_name}${GREEN}'${NC}"
else
    echo -e "${ORANGE}Skipping systemd service installation.${NC}"
    echo -e "${ORANGE}You can start ${service_name} manually by running 'bash scripts/start.sh'${NC}"
fi

echo -e "${GREEN}${service_name} has been installed successfully!${NC}"
